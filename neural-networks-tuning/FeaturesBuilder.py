...
class FeaturesBuilder:
    ...
    def build_avg_tweet_embedding(self):
        ...
        valid_features = np.sum(np.isnan(features), axis=1) == 0
        print('built features with shape', features[valid_features].shape)
        return features[valid_features], labels[valid_features]
    ...
    def _pad_with_zeros(self, tweet):
        words = tweet.split()
        sequence = np.array([self._get_vocab_idx(word) for word in words])
